...
public class UCompareWindow extends JFrame {
	 ...
	 * @param toDisplay
	 *            Container to display in the test window
	 */
	public UCompareWindow(String windowTitle, Container toDisplay) {
		...
		final Preferences prefs = Preferences.userNodeForPackage(this
				.getClass());
		...
		int prefHeight = prefs.getInt(UCompareWindow.PREF_HEIGHT,
				UCompareWindow.PREFERRED_SIZE.height);
		int prefWidth = prefs.getInt(UCompareWindow.PREF_WIDTH,
				UCompareWindow.PREFERRED_SIZE.width);
		...
		if (MINIMUM_SIZE.getWidth() > bounds.getWidth()
			...
			JOptionPane.showMessageDialog(
					...
					"We require at least resolution of "
							...
							+ (int) MINIMUM_SIZE.getHeight()
							+ ", you are using " + (int) bounds.getWidth()
							+ "x" + (int) bounds.getHeight(), "Error",
					JOptionPane.ERROR_MESSAGE);
			System.exit(-1);
		}

		int xPos = (int) Math.max(0, (bounds.getWidth() - this.getWidth()) / 2);
		int yPos = (int) Math.max(0,
				(bounds.getHeight() - this.getHeight()) / 2);

		if (UCompareWindow.DEBUG) {
			System.err.println(this.getClass().getName()
					+ ": Will center on co-ordinates x: " + xPos + " y: "
					+ yPos);
		}
