...
/**
 * A border with rounded edges. Used in place of LineBorder with rounded corners
 * as it gives more flexibility in setting the degree of corner rounding.
 * 
 */
public class RoundedBorder implements Border {
	 ...
	 * 
	 * @param background
	 *            if set to null will be transparent.
	 * @param outline
	 * @param foreground
	 * @param radius
	 * @param stroke
	...
	public void paintBorder(Component c, Graphics g, int x, int y, int width,
		...
		// fill corners
		int arcLeft = 0;
		int arcTop = 1;// = 0; //Removes faint foreground color ghosting on
						// corner
		...
		// fill center
		g2d.fillRect(radius, radius, sideWidth, sideHeight);
		// fill sides
		...
		if (stroke > 0) {
			...
			g2d.drawArc(leftStroke, topStroke, strokeDiameter, strokeDiameter,
					90, 90);
			g2d.drawArc(rightStroke, topStroke, strokeDiameter, strokeDiameter,
					0, 90);
	...
	private static class TestRunnable implements Runnable {
		public void run() {
			...
			cell.add(new JLabel("<html><i>radius</i>: " + p
					+ "<br><i>stroke</i>: " + s));
			cell.setBorder(new RoundedBorder(background, border, foreground, p,
					s, true));
