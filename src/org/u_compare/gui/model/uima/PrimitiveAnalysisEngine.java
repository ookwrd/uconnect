...
public class PrimitiveAnalysisEngine extends AbstractComponent {
	...
	public PrimitiveAnalysisEngine() {
		...
		try {
			 ...
			 * .parseAnalysisEngineDescription(xmlIn); casConsumerDesc =
			 * UIMAFramework.getXMLParser() .parseCasConsumerDescription(xmlIn);
			 */
			// etc

			/*
			 * //Can i use this instead? ResourceSpecifier specifier =
			 * UIMAFramework.getXMLParser().parseResourceSpecifier(xmlIn);
			 */

			/**
			 * this is a nasty nasty way of doing this, but it is what I found
			 * done in AnnotationViewerMain.viewDocuments() as shipped with
			 * UIMA. The problem is the input xml could represent multiple
			 * types, analysisEngine, casConsumer, etc but we either need to
			 * specify a specific parse method on the XMLParser or a specific
			 * method on the UIMAFramework when we construct an actual instance
			 * from it. There should be a nicer way than this... TODO find it or
			 * consult the UIMA mailing list
			 */
			Object resourceSpecifier = UIMAFramework.getXMLParser()
					.parse(xmlIn);
			...
			if (resourceSpecifier instanceof AnalysisEngineDescription) {
				...
				// ResourceMetaData metaData = desc.getMetaData();//Doesn't get
				// us all the methods we need.
				AnalysisEngineMetaData metaData = desc
						.getAnalysisEngineMetaData();// Also exists Processing
														// Resource metaData,
														// maybe the level we
														// want
				...
				for (String key : desc.getDelegateAnalysisEngineSpecifiers()
						.keySet()) {
					System.out.println("Key: " + key);
					ResourceSpecifier specifier = desc
							.getDelegateAnalysisEngineSpecifiers().get(key);
				...
				TypeSystemDescription typeSystemDescription = metaData
						.getTypeSystem();
				...
				for (Capability capability : metaData.getCapabilities()) {
					System.out.println(capability.getDescription());// TODO how
																	// the hell
																	// do I set
																	// a
																	// capability
				...
				System.out.println("Deafult Group name: "
						+ declarations.getDefaultGroupName());
				System.out.println("Search Stratergy: "
						+ declarations.getSearchStrategy());
				ConfigurationGroup[] groups = declarations
						.getConfigurationGroups();
				if (groups != null && groups.length != 0) {
					ConfigurationGroup group = groups[0];
					System.out.println("Group names: " + group.getNames());
					for (ConfigurationParameter parameter : group
							.getConfigurationParameters()) {
						System.out.println("Name: " + parameter.getName());
						System.out.println("Value: "
								+ settings.getParameterValue(
										group.getNames()[0],
										parameter.getName()));
					}
				}
				...
				for (ConfigurationParameter param : metaData
						.getConfigurationParameterDeclarations()
				 ...
				 * settings.getParameterSettings()){ System.out.println("Name: "
				 * + pair.getName()); System.out.println("Value: " +
	...
	/*
	 * public PrimitiveUIMAComponent(String descriptorLocation) throws
	 * IOException, InvalidXMLException { this(new
	 * XMLInputSource(descriptorLocation)); }
	 * 
	 * public PrimitiveUIMAComponent(XMLInputSource inputSource) throws
	 * InvalidXMLException{ try { Object resourceSpecifier =
	 ...
	 * public static void main(String[] args){
	 * 
	 * try { Component component = AbstractComponent.constructComponentFromXML(
	 * "src/org/u_compare/gui/model/uima/debugging/" +
	 * "basicCollectionReaderDescriptor.xml"); } catch (InvalidXMLException e) {
