...
/**
 * Extension of JTabbedPane which allows the inclusion of ButtonTabs which can
 * not be selected, but which act as JButtons.
 * 
...
public class ButtonTabbedPane extends JTabbedPane {
	...
	public ButtonTabFlap addButtonTab(String title,
			DropTargetListener dropListener) {
		ButtonTabFlap tabFlap = new ButtonTabFlap(title, dropListener);

		// Create a component that whenever it is displayed switches the tab to
		// a previous one.
		addTab(title, new JLabel() {
			public void paint(Graphics g) {// Makes the tab undisplayable, on
											// attempts to draw it opens the
											// last non-button
				switchToLastTab();
			}
		});
	...
	public int numberOfNonButtonTabs() {
		return super.getTabCount() - (buttons == null ? 0 : buttons.size())
				- (zeroInUse() ? 1 : 0);// Somehow this seems to be called at
										// construction time prior to
