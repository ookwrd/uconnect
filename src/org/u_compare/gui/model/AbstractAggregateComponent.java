...
/**
 * Abstract base class implementing functionality common to all aggregate
 * components.
 * 
 * @author Luke McCrohon
 */
public abstract class AbstractAggregateComponent extends AbstractComponent
		implements AggregateComponent {

	@SuppressWarnings("serial")
	public class InvalidPositionException extends Exception {
		public InvalidPositionException(int position, int maxPosition) {
			super("Tried to add component in position " + position
					+ ", valid positions are 0<=x<=" + maxPosition);
		}
	}
	...
	/**
	 * Adds the passed component to the aggregate at the specified position.
	 * Position must be between 0 and the number of components. If a component
	 * is inserted in an intermediary position, subsequent components will be
	 * moved down.
	 * 
	 * @param position
	 *            to insert at
	 ...
	 */
	@Override
	public void addSubComponent(int position, Component component)
	...
	/**
	 * Adds the passed component to the list of subcomponents if it is not
	 * already present. Will be added to end by default.
	 * 
	 * @param component
	 *            to add
	 */
	...
	@Override
	public void registerSubComponentsChangedListener(
			SubComponentsChangedListener listener) {
	...
	// The following methods default to always allowing modifications to the
	// model, override this behaviour when appropriate.
	@Override
	...
	public ResourceCreationSpecifier getResourceCreationSpecifier() {
		AnalysisEngineDescription retVal = (AnalysisEngineDescription) super
				.getResourceCreationSpecifier();
		...
		if (flowConstraints instanceof FixedFlow) {
			...
			for (int i = 0; i < subComponents.size(); i++) {
				...
				if (subComponents.get(i).getFlowControllerIdentifier() != null) {
					String current = subComponents.get(i)
							.getFlowControllerIdentifier();
			...
			retVal.getAnalysisEngineMetaData().setFlowConstraints(
					flowConstraints);
		}

		retVal.setFlowControllerDeclaration(flowController);
		if (flowController != null) {
			System.err.println("Flow controller not null. Specifier type "
					+ flowController.getSpecifier());
		 ...
		 * //metaData.put(comp.getName(),comp.getUIMADescription()); //TODO if I
		 * do it this way can I run?? //TODO obviously
		 * 
		 * /* So can I avoid saving everything to the file system piecemeal by
		 * making use of components with their subcomponents built?
		 * 
		 * Similarly, can I save the xml like this in a single file? Rather than
		 * seperately.
		 * 
		 * Lets try.
		 */
		...
		for (Component comp : getSubComponents()) {
			metaData.put(comp.getFlowControllerIdentifier(),
					comp.getResourceCreationSpecifier());
		}
