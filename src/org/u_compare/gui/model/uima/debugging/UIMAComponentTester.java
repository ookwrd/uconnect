...
public class UIMAComponentTester {
	...
	@Test
	public void inOutTestCPEimport() throws InvalidXMLException, IOException,
			CpeDescriptorException {
		inOutTestCPE("src/org/u_compare/gui/model/uima/debugging/CPEimport.xml");
	}

	@Test
	public void inOutTestCPEdirect() throws InvalidXMLException, IOException,
			CpeDescriptorException {
		inOutTestCPE("src/org/u_compare/gui/model/uima/debugging/CPEdirect.xml");
	}

	/**
	 * Performs a test to ensure the XML produced from loading a XMLDescriptor
	 * into the model and then extracting it again matches the original XML.
	 * 
	 * Currently uses string matching to determine if a correct match is
	 * produced. This will sometimes produced false negatives. Should be
	 * replaced with an XML comparator TODO
	 * 
	 * @param location
	 *            The location of the XMLDescriptor to test.
	 * @throws IOException
	 * @throws InvalidXMLException
	 */
	private void inOutTest(String location) throws IOException,
			InvalidXMLException {
		XMLInputSource xmlIn = new XMLInputSource(location);
		ResourceSpecifier desc = (ResourceSpecifier) UIMAFramework
				.getXMLParser().parse(xmlIn);

		Component component = AbstractComponent
				.constructComponentFromXML(location);

		assertTrue(compare(descriptorToString(desc),
				descriptorToString(component.getResourceCreationSpecifier())));
	}

	private void inOutTestCPE(String location) throws IOException,
			InvalidXMLException, CpeDescriptorException {
		XMLInputSource xmlIn = new XMLInputSource(location);
		CpeDescription desc = (CpeDescription) UIMAFramework.getXMLParser()
				.parse(xmlIn);

		CPE workflow = new CPE(desc);

		assertTrue(compare(descriptorToString(desc),
				descriptorToString(workflow.getWorkflowDescription())));
	}
	...
	public static boolean compare(String target, String result) {
		...
		if (target.equals(result)) {
			...
			System.out.println("Matches the first " + i + " characters (of "
					+ target.length() + "):");
			System.out.print(result.substring(0, i) + "\nXXXXXXXXXXXXXXXX\n"
					+ result.substring(i, result.length()));
			System.out.println("\nTarget:");
			System.out.println(target.substring(i - 10 > 0 ? i - 10 : 0,
					i + 100 < target.length() ? i + 100 : target.length()));
			System.out.println("\nProduced:");
			System.out.println(result.substring(i - 10 > 0 ? i - 10 : 0,
					(i + 100 < result.length() ? i + 100 : result.length())));
